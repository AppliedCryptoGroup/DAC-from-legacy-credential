...
pub fn dummy_circuit<F: RichField + Extendable<D>, C: GenericConfig<D, F = F>, const D: usize>(
    ...
    // Number of `NoopGate`s to add to get a circuit of size `degree` after blinding & padding.
    let num_noop_gate = num_dummy_noop_gates::<F, D>(common_data);
...
fn num_dummy_noop_gates<F: RichField + Extendable<D>, const D: usize>(
    common_data: &CommonCircuitData<F, D>,
) -> usize {
    let degree = common_data.degree();
    // Account for public input hashing, a `PublicInputGate`, and a `ConstantGate`.
    let base_gate_overhead = common_data.num_public_inputs.div_ceil(8) + 2;
    let max_noop_gate = degree.saturating_sub(base_gate_overhead);

    if !common_data.config.zero_knowledge {
        return max_noop_gate;
    }

    // Approximation strategy:
    // 1) Assume a noop count (binary search) and compute the pre-blinding gate count.
    // 2) Estimate the final degree after blinding + padding using the same math as the builder.
    // 3) Adjust the noop count until the estimated final degree matches `common_data.degree()`.
    // This works because the final degree is monotonic in the number of pre-blinding gates.
    let mut lo = 0usize;
    let mut hi = max_noop_gate;
    let mut best = None;
    while lo <= hi {
        let mid = (lo + hi) / 2;
        let pre_blind_gates = base_gate_overhead + mid;
        // Simulate builder's blinding math to see the final degree we would get.
        let final_degree = degree_after_blinding::<D>(&common_data.config, pre_blind_gates);
        if final_degree == degree {
            best = Some(mid);
            lo = mid + 1;
        } else if final_degree < degree {
            lo = mid + 1;
        } else {
            if mid == 0 {
                break;
    ...
    best.unwrap_or_else(|| {
        panic!(
            "Unable to match dummy circuit degree {} with zero-knowledge enabled",
            degree
        )
    })
}

fn degree_after_blinding<const D: usize>(config: &CircuitConfig, num_gates: usize) -> usize {
    // Start from the closest power-of-two estimate and grow until blinding fits.
    ...
    loop {
        // Mirrors CircuitBuilder::num_blinding_gates to estimate the extra zk gates.
        // The count depends on degree_estimate (FRI params), so we loop to a fixed point.
        // TODO: Check if we need to account for LUT blinding as well.
        let (regular_poly_openings, z_openings) = num_blinding_gates::<D>(config, degree_estimate);
...
fn num_blinding_gates<const D: usize>(
    config: &CircuitConfig,
    degree_estimate: usize,
) -> (usize, usize) {
    let degree_bits_estimate = log2_strict(degree_estimate);
    let fri_queries = config.fri_config.num_query_rounds;
    // This tracks the same derivation as CircuitBuilder::num_blinding_gates.
    let arities: Vec<usize> = config
        .fri_config
        .fri_params(degree_bits_estimate, config.zero_knowledge)
    ...
    let fri_openings =
        fri_queries * (1 + D * total_fri_folding_points + D * final_poly_coeffs);
