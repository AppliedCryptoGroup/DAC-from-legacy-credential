...
pub struct CommonCircuitData<F: RichField + Extendable<D>, const D: usize> {
    ...
    /// Gate row count after blinding but **before** NoopGate padding to a power
    /// of two. Diagnostic-only — excluded from [`PartialEq`] so cyclic-recursion
    /// goal matching (which compares `CommonCircuitData` values built from
    /// different builders) is unaffected.
    pub num_gates_pre_padding: usize,
}

impl<F: RichField + Extendable<D>, const D: usize> PartialEq for CommonCircuitData<F, D> {
    fn eq(&self, other: &Self) -> bool {
        // `num_gates_pre_padding` is intentionally excluded: it's a diagnostic
        // count that varies between two builders producing the same circuit
        // shape (e.g. when the goal CCD is reconstructed without rebuilding).
        self.config == other.config
            && self.fri_params == other.fri_params
            && self.gates == other.gates
            && self.selectors_info == other.selectors_info
            && self.quotient_degree_factor == other.quotient_degree_factor
            && self.num_gate_constraints == other.num_gate_constraints
            && self.num_constants == other.num_constants
            && self.num_public_inputs == other.num_public_inputs
            && self.k_is == other.k_is
            && self.num_partial_products == other.num_partial_products
            && self.num_lookup_polys == other.num_lookup_polys
            && self.num_lookup_selectors == other.num_lookup_selectors
