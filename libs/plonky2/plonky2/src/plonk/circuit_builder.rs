...
impl<F: RichField + Extendable<D>, const D: usize> CircuitBuilder<F, D> {
    ...
    /// Set a goal [`CommonCircuitData`] that the built circuit must match.
    ///
    /// If set, [`Self::try_build_with_options`] will return `success = false` when the
    /// resulting `CommonCircuitData` does not equal the goal.
    ///
    /// **Note:** This does *not* pad the circuit automatically.  The caller is
    /// responsible for adding enough `NoopGate`s and ensuring the gate set matches.
    pub fn set_goal_common_data(&mut self, goal: CommonCircuitData<F, D>) {
        self.goal_common_data = Some(goal);
    }
    ...
    /// Apply blinding (when ZK is enabled) and pad gate instances out to a
    /// power of two. Returns the gate count *after* blinding.
    fn blind_and_pad(&mut self) -> usize {
        if self.config.zero_knowledge {
            self.blind();
        }

        let num_gates_pre_padding = self.gate_instances.len();

        while !self.gate_instances.len().is_power_of_two() {
            self.add_gate(NoopGate, vec![]);
        }

        num_gates_pre_padding
    }
    ...
    pub fn try_build_with_options<C: GenericConfig<D, F = F>>(
        ...
        let num_gates_pre_padding = self.blind_and_pad();
        let degree = self.gate_instances.len();
